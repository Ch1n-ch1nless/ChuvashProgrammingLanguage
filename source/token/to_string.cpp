...
std::string toString(const TokenVariant &token) {
  return std::visit(
    overloaded {
      []<LogicalOperator LogicalOperatorT>([[maybe_unused]] const LogicalOperatorT& op) {
        return std::format("LogicalOperator: {}", ::toStringUnqualified<LogicalOperatorT>());
      },
      []<Operator OperatorT>([[maybe_unused]] const OperatorT& op) {
        return std::format("Operator: {}", ::toStringUnqualified<OperatorT>());
      },
      ...
      []<Keyword KeywordT>([[maybe_unused]] const KeywordT& token) {
        return std::format("Keyword: {}", ::toStringUnqualified<KeywordT>());
      },
