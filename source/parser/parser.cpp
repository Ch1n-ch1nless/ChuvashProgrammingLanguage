...
std::expected<ExpressionVariant, std::string> parseTerm(TokenIterator &it,
  ...
  while (it != end) {
    ...
    if (isPlus.value()) {
      left = Addition{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
    } else {
      left = Subtraction{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
    }
  }
  return left;
}
...
std::expected<ExpressionVariant, std::string> parseEquality(TokenIterator &it,
  ...
  while (it != end) {
    ...
    if (isEqual.value())
      left = Equal{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
    else
      left = NotEqual{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
  }
  return left;
}
...
std::expected<ExpressionVariant, std::string> parseLogicalAnd(TokenIterator &it,
  ...
  while (it != end && std::holds_alternative<token::And>(it->token)) {
    ...
    if (!right) return std::unexpected(right.error());
    left = And{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
  }
  return left;
}
...
std::expected<ExpressionVariant, std::string> parseLogicalOr(TokenIterator &it,
  ...
  while (it != end) {
    ...
    ExpressionVariant newNode;
    if (op->kind == 0)
      newNode = Or{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
    else
      newNode = Xor{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
    left = std::move(newNode);
  }
  return left;
}
...
std::expected<ExpressionVariant, std::string> parseAssignment(TokenIterator &it,
  ...
  if (it != end && std::holds_alternative<token::Assign>(it->token)) {
    ...
    if (!right) return std::unexpected(right.error());
    return Assign{ExprPtr(std::move(*left)), ExprPtr(std::move(*right))};
  }
  return left;
}
...
std::expected<ExpressionVariant, std::string> parsePostfix(TokenIterator &it,
  ...
  while (it != end &&
    ...
    auto rparen = expectToken<token::RightParenthesis>(it, end);
    if (!rparen) return std::unexpected(rparen.error());
    left = Call{ExprPtr(std::move(*left)), std::move(args)};
  }
  return left;
}
  ...
  return std::visit(
      overloaded{
          ...
          [&](const token::Minus &)
            ...
            auto operand = parseUnary(it, end, pos);
            if (!operand) return std::unexpected(operand.error());
            return UnaryMinus{ExprPtr(std::move(*operand))};
          },
          [&](const auto &) -> std::expected<ExpressionVariant, std::string> {
...
std::expected<ScopeStatement, std::string> parseBlock(TokenIterator &it,
  ...
  while (it != end && !std::holds_alternative<token::RightBrace>(it->token)) {
    ...
    stmts.push_back(StatePtr(std::move(*stmt)));
  }
  auto rbrace = expectToken<token::RightBrace>(it, end);
  if (!rbrace) return std::unexpected(rbrace.error());
